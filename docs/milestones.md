...
- [ ] 主網 Safe singleton 的 code hash 與以太坊部署版本核對（M7 稽核）
...
- [ ] RandomX 自訂參數版本（`randomx-rs`），light mode 驗證、fast mode 挖礦；種子每 2,048 塊更換
- [ ] header 封印（`nonce`、`difficulty`）、ASERT 難度調整，目標出塊間隔 12 秒
- [ ] 分叉選擇（累積難度）、儲存層倒回與規範鏈切換，自動重組深度上限 128 塊
- [ ] PoW 同步路徑（IPFS 公告不帶最終性證明，驗證 PoW 與累積難度）
- [ ] `boltchain mine`；階段 A 的出塊獎勵發給礦工
- [ ] 移除啟動期驗證者、鎖倉獎勵、鎖倉權重上限；更新 genesis 格式與主網範本
- [ ] 移除治理權限（ADR 0008）：Safe、timelock、proxy 與暫停權限；系統合約改為不可升級，管理函式只接受 system 地址
- [ ] gasLimit 由出塊者投票（每塊 ±1/1024，範圍 15M–60M）；其他參數改為協定常數
- [ ] 硬分叉表與不規則狀態變更的框架；分叉 ID 納入 P2P 協定，規則不相容的節點不互連
- [ ] 階段 A → C 轉換（達 T2 並連續維持 14 個 epoch；終端區塊；首個委員會只從質押滿 14 個 epoch 的驗證者中抽選）
- [ ] 模擬器：租來的多數算力、自私挖礦、網路分區、轉換點奪取
- [ ] 驗收：5 個礦工節點 + 10 個跟隨節點；51% 攻擊下重組上限生效；從 genesis 同步會驗證 PoW；自動切換到 PoS
