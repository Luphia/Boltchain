...
> **2026-09-25：啟動期驗證者相關部分（§2 的啟動期委員會、§8 的 25% 規則、§11 鎖倉權重上限）已被 ADR 0007 取代；§7 的治理設計（Safe、timelock、UUPS 升級、暫停權限、可調參數）已被 ADR 0008 取代。** epoch、委員會抽籤、nil 收尾交接、罰沒與系統合約的其餘設計仍然有效，會用在階段 B 與階段 C。
...
2. 若本區塊是 `start(e)` 且 `e ≥ 1`：
   ...
   3. 存儲獎勵要到 M5 才有抽查機制，M4 先不鑄造（金額後來改為每塊 1 BOLT，見 ADR 0012）。
