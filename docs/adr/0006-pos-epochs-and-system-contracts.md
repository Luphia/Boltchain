...
依方案，啟動期驗證者拿 25% 的投票獎勵，而且自動質押鎖倉。實作後發現兩個後果：

- **退出條件幾乎由他們自己達成**：以方案的參數，7 位啟動期驗證者每天共鎖入約 40.8 萬 BOLT，約 25 天就能讓「總質押 ≥ 1,000 萬」成立。「至少 128 個質押者」這個條件也把他們算進去。
- **退出後他們握有大部分質押**：委員會依質押權重抽籤，前幾個月的委員會會由這 7 位主導，等於延長了聯盟鏈階段。M4 驗收測試中，當外部驗證者各只押約 100 BOLT 時，抽出的委員會全部是啟動期驗證者。

目前的實作完全照方案（鎖倉獎勵算作一般質押）。可以考慮的調整：

1. 退出條件只計算非啟動期驗證者的人數與質押。
2. 啟動期結束後，鎖倉獎勵在抽籤時打折或設權重上限，或分幾個 epoch 逐步計入。
3. 降低或取消啟動期獎勵，鎖倉部分改成燃燒。

這是經濟模型的決定，需要專案負責人拍板；M7 主網參數定案前必須決定。
...
- 端對端（`crates/node/tests/m4_pos.rs`）：
  ...
  - epoch 2–6 的委員會各不相同，共有數十位驗證者輪流上任；
